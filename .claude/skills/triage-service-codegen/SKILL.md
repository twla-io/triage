...
- **Named outcome types** (`MatchOutcome`, ...): a legitimate branch of business logic the caller reacts to, typically differently per constructor — never an error, never folded into `Left`. Live cases: `NoEligibleRequest`, `SlotAlreadyClaimed`, `RequestAlreadyClaimed`.
